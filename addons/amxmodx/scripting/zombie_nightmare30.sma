...
g_msgScreenShake, g_msgCurWeapon, g_msgFog
...
cvar_thunder, cvar_zombiebonushp, cvar_deathmatch, cvar_hitzones, cvar_humanhp, cvar_flashsize,
...
{	
	...
	{
		health = 100 * health / ArrayGetCell(g_zclass_hp, g_zombieclass[msg_entity]);
	}
...
public message_teaminfo(msg_id, msg_dest)
{
	...
	switch (team[0])
	{
		case 'C': // CT
		{
			if (!fnGetZombies() && !g_newround && !g_endround) // no zombies alive --> switch to T and spawn as zombie
			{
		...
		case 'T': // Terrorist
		{
			if (fnGetZombies() || g_newround) // zombies alive --> switch to CT
			{
				remove_task(id+TASK_TEAM)
				fm_cs_set_user_team(id, FM_CS_TEAM_CT)
...
public returnammo()
{
	if (!get_pcvar_num(cvar_retammo))
	{
		return;
	}
	for(new i = 1; i <= g_maxplayers; i++)
	{
		if(!g_isconnected[i] || !g_zombie[i])
			continue;
		
		if(g_return_money[i][0] > g_ammopacks[i])
		{
			g_ammopacks[i] = g_return_money[i][0];
		}
		if(g_return_money[i][1] > g_souls[i])
		{
			g_souls[i] = g_return_money[i][1];
		}
	}
}
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		if (fnGetHumans() == 1 && fnGetPlayersCount(FM_CS_TEAM_CT) == 1)
			return;
...
// Get CTs -returns number of team players connected-
fnGetPlayersCount(team)
{
	static iCTs, id
	iCTs = 0
	for (id = 1; id <= g_maxplayers; id++)
